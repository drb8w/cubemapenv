...
namespace TotalGlobal
{
	...
	bool CubemapFramebuffer::BeginRendering()
	{
		...
		glActiveTexture(GL_TEXTURE0 + m_Index_texUnit);
		glBindTexture(GL_TEXTURE_CUBE_MAP, m_Index_color_texture);  // necessary  because of call to glFramebufferTexture2D later ???
#ifdef USE_DEPTH_BUFFER
		glBindTexture(GL_TEXTURE_CUBE_MAP, m_Index_depth_texture);   // necessary and correct to bind to the same texture unit ???
#endif
